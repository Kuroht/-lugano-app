...
export default function MenuTab({products, ingredients, message}) {
  ...
  const getMenuPage = () => {
    switch (currentPage) {
      case "pizza":
        return <MenuPage title={message.pizza} contents={productsArr} message={message}/>;
      case "pasta":
        return <MenuPage title={message.pasta} contents={productsArr} message={message}/>;
      case "ingredient":
        return <MenuPage title={message.ingredient} contents={ingredients} message={message}/>;
      case "other":
        return <MenuPage title={message.other} contents={productsArr} message={message}/>;
      default:
  ...
  function productsByType(type: string) {
    ...
    switch (type.toLocaleUpperCase()) {
      case "PIZZA":
        filteredProducts = products.filter((product : Product) => product.type.toLocaleUpperCase() === "PIZZA");
        break;
      case "PASTA":
        filteredProducts = products.filter((product : Product) => ["MASSA", "CARNE", "SALADA"].includes(product.type.toLocaleUpperCase()));
        break;
      case "OTHER":
        filteredProducts = products.filter((product : Product) => product.type.toLocaleUpperCase() === "EXTRAS");
        break;
      default:
